import{r as B,j as n}from"./index-CEPlWTXw.js";import{C as M,V as _,r as ae,H as re,U as he,s as G,g as N,t as Be,h as _e,R as Re,S as Pe,P as Fe,W as Ue,u as ze,v as je,O as Ee,A as Ne,w as Ae,D as He,x as ke,y as Le,M as W,b as Ve,z as Oe,E as me,l as de,J as Ie,K as Qe,c as Ye,o as Ge,N as We,q as $e,n as Ke}from"./OrbitControls-HQYc4E0y.js";import{P as Xe,C as oe,F as qe,E as Je,R as Ze}from"./RenderPass-qdSimjnd.js";const et={uniforms:{tDiffuse:{value:null},luminosityThreshold:{value:1},smoothWidth:{value:1},defaultColor:{value:new M(0)},defaultOpacity:{value:0}},vertexShader:`
		...
		void main() {
			...
			gl_FragColor = mix( outputColor, texel, alpha );
				...
				}`})}}A.BlurDirectionX=new _(1,0);A.BlurDirectionY=new _(0,1);const fe={note_created:"#3B82F6",note_confirmed:"#4ade80",decision:"#8B5CF6",commit:"#64748B",skill_created:"#EC4899",skill_activated:"#fbbf24",protocol_transition:"#F97316"},pe=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"],ge=new M(2282478);function rt({events:H,projectColorMap:e}){const p=B.useRef(null),l=B.useRef(0),a=B.useRef(null),o=B.useRef(new Re),m=B.useRef(new _(-999,-999)),[c,d]=B.useState(null),{grid:L,perProjectGrid:T,maxCount:P,eventsByCell:U,dayTotals:$,sortedSlugs:K}=B.useMemo(()=>{const r=Array.from({length:7},()=>Array(24).fill(0)),u=Array.from({length:7},()=>Array.from({length:24},()=>new Map)),h=new Map;for(const f of H){const D=f.date.getDay(),v=f.date.getHours();r[D][v]++;const V=`${D}-${v}`,O=h.get(V)||[];if(O.push(f),h.set(V,O),e){const S=f.projectSlug||"_global",I=u[D][v];I.set(S,(I.get(S)||0)+1)}}let i=0;for(const f of r)for(const D of f)D>i&&(i=D);const b=r.map(f=>f.reduce((D,v)=>D+v,0)),g=e?Array.from(new Set(H.map(f=>f.projectSlug||"_global"))).sort():[];return{grid:r,perProjectGrid:u,maxCount:i||1,eventsByCell:h,dayTotals:b,sortedSlugs:g}},[H,e]),X=B.useCallback(r=>{if(!p.current)return;const u=p.current.getBoundingClientRect();m.current.x=(r.clientX-u.left)/u.width*2-1,m.current.y=-((r.clientY-u.top)/u.height)*2+1},[]),q=B.useCallback(()=>{if(m.current.set(-999,-999),d(null),a.current){const r=a.current.userData,u=a.current.material;u.emissiveIntensity=r.baseEmissive,a.current=null}},[]);return B.useEffect(()=>{const r=p.current;if(!r)return;const u=r.clientWidth,h=360,i=new Pe;i.background=new M(527380);const b=new Fe(45,u/h,.1,200);b.position.set(20,22,20),b.lookAt(12,0,3.5);const g=new Ue({antialias:!0,alpha:!1});g.setSize(u,h),g.setPixelRatio(Math.min(window.devicePixelRatio,2)),g.shadowMap.enabled=!0,g.shadowMap.type=ze,g.toneMapping=je,g.toneMappingExposure=1.4,r.appendChild(g.domElement);const f=new Je(g);f.addPass(new Ze(i,b));const D=new A(new _(u,h),.4,.6,.9);f.addPass(D);const v=new Ee(b,g.domElement);v.enableDamping=!0,v.dampingFactor=.05,v.target.set(12,0,3.5),v.minDistance=10,v.maxDistance=60,v.maxPolarAngle=Math.PI/2.1,v.update();const V=new Ne(6583435,1.5);i.add(V);const O=new Ae(2282478,1976635,.6);i.add(O);const S=new He(13358561,.5);S.position.set(-10,15,10),S.castShadow=!0,S.shadow.mapSize.set(1024,1024),S.shadow.camera.left=-15,S.shadow.camera.right=30,S.shadow.camera.top=12,S.shadow.camera.bottom=-5,i.add(S);const I=new ke(40,20),xe=new Le({color:791074,roughness:.95,metalness:.05}),Q=new W(I,xe);Q.rotation.x=-Math.PI/2,Q.position.set(12,-.01,3.5),Q.receiveShadow=!0,i.add(Q);const ie=new Ve(40,40,1450034,988964);ie.position.set(12,0,3.5),i.add(ie);const J=[],Z=new Oe(.85,1,.85);Z.translate(0,.5,0);const be=!!e&&K.length>1;for(let s=0;s<7;s++)for(let t=0;t<24;t++){const w=L[s][t],x=w/P,y=w===0?.12:.25+x*7,R=w===0;if(be&&!R){const z=T[s][t];let k=0;for(const j of K){const E=z.get(j)||0;if(E===0)continue;const te=E/w,C=y*te,Y=E/P,Te=e.get(j)||"#64748B",se=new M(Te),Me=se.clone().multiplyScalar(.4),ce=.5+Y*.45,Se=Math.max(.05,.45-Y*.4),ue=.15+Y*.4,Ce=new M().lerpColors(Me,se,Math.max(.4,Y)),De=new me({color:Ce,emissive:se.clone(),emissiveIntensity:ue,transparent:!0,opacity:ce,transmission:Se,roughness:.15,metalness:0,thickness:.8,ior:1.4,envMapIntensity:.5,side:de}),F=new W(Z,De);F.position.set(t,k,s),F.scale.y=C,F.castShadow=!0,F.receiveShadow=!0,F.userData={day:s,hour:t,count:w,segCount:E,projectSlug:j,intensity:x,baseEmissive:ue,baseScaleY:C,baseOpacity:ce,baseY:k},i.add(F),J.push(F),k+=C}}else{const z=R?.25:.4+x*.5,k=R?.8:Math.max(.05,.55-x*.5),j=R?.05:.1+x*.45,E=new M().lerpColors(new M(1400437),ge,R?.15:Math.max(.3,x)),te=new me({color:E,emissive:ge.clone(),emissiveIntensity:j,transparent:!0,opacity:z,transmission:k,roughness:.15,metalness:0,thickness:.8,ior:1.4,envMapIntensity:.5,side:de}),C=new W(Z,te);C.position.set(t,0,s),C.scale.y=y,C.castShadow=!R,C.receiveShadow=!0,C.userData={day:s,hour:t,count:w,intensity:x,baseEmissive:j,baseScaleY:y,baseOpacity:z},i.add(C),J.push(C)}}const we=Math.max(...$,1),ee=$.map((s,t)=>({day:t,total:s,t:s/we})).filter(s=>s.total>0).sort((s,t)=>t.total-s.total).slice(0,3);for(const{day:s,t}of ee){if(t<.4)continue;const w=1.5+t*3,x=new M().lerpColors(new M(959977),new M(14742270),t*.6),y=new Ie(x,w,30,Math.PI/5,.6,1.2);y.position.set(11.5,14,s),y.target.position.set(11.5,0,s),y.castShadow=!0,y.shadow.mapSize.set(512,512),i.add(y),i.add(y.target)}if(ee.length===0||ee[0].t<.4){const s=new Qe(2282478,1,50);s.position.set(12,14,3.5),i.add(s)}for(let s=0;s<7;s++){const t=ve(pe[s],{fontSize:28,color:"#64748b"});t.position.set(-1.5,.1,s),t.scale.set(1.6,.5,1),i.add(t)}for(let s=0;s<24;s+=3){const t=ve(`${s}h`,{fontSize:24,color:"#475569"});t.position.set(s,.1,7.5),t.scale.set(1.2,.4,1),i.add(t)}r.addEventListener("mousemove",X),r.addEventListener("mouseleave",q);const ye=()=>{if(!r)return;const s=r.clientWidth;b.aspect=s/h,b.updateProjectionMatrix(),g.setSize(s,h),f.setSize(s,h)},ne=new ResizeObserver(ye);ne.observe(r);const le=()=>{l.current=requestAnimationFrame(le),v.update(),o.current.setFromCamera(m.current,b);const s=o.current.intersectObjects(J);if(a.current){const t=a.current.userData,w=a.current.material;w.emissiveIntensity=t.baseEmissive,w.opacity=t.baseOpacity,a.current.scale.y=t.baseScaleY,a.current=null}if(s.length>0){const t=s[0].object;if(t.userData.count>0){a.current=t;const w=t.material;w.emissiveIntensity=Math.min(t.userData.baseEmissive+.35,.9),w.opacity=Math.min(t.userData.baseOpacity+.2,1),t.scale.y=t.userData.baseScaleY*1.06;const x=new N;x.copy(t.position),x.y=(t.userData.baseY??0)+t.scale.y+.5,x.project(b);const y=r.getBoundingClientRect(),R=(x.x+1)/2*y.width,z=(-x.y+1)/2*y.height;d({x:R,y:z,day:pe[t.userData.day],hour:t.userData.hour,count:t.userData.count,events:U.get(`${t.userData.day}-${t.userData.hour}`)||[]})}else d(null)}else d(null);f.render()};return le(),()=>{cancelAnimationFrame(l.current),r.removeEventListener("mousemove",X),r.removeEventListener("mouseleave",q),ne.disconnect(),v.dispose(),g.dispose(),f.dispose(),i.traverse(s=>{s instanceof W&&(s.geometry.dispose(),s.material instanceof Ye&&s.material.dispose())}),r.contains(g.domElement)&&r.removeChild(g.domElement)}},[L,T,P,U,$,K,e,X,q]),n.jsxs("div",{className:"relative",children:[n.jsx("div",{ref:p,className:"w-full rounded-lg overflow-hidden",style:{height:360,cursor:"grab"}}),c&&n.jsx("div",{className:"absolute z-50 pointer-events-none",style:{left:c.x,top:c.y,transform:"translate(-50%, -110%)"},children:n.jsxs("div",{className:"bg-slate-900/95 backdrop-blur-sm border border-slate-700/80 rounded-lg px-3 py-2 shadow-xl min-w-[180px] max-w-[260px]",children:[n.jsxs("div",{className:"flex items-center justify-between mb-1.5",children:[n.jsxs("span",{className:"text-[10px] font-medium text-cyan-400",children:[c.day," ",c.hour,":00–",c.hour+1,":00"]}),n.jsxs("span",{className:"text-[10px] text-slate-500 font-mono",children:[c.count," event",c.count!==1?"s":""]})]}),n.jsx("div",{className:"space-y-1",children:e?(()=>{const r=new Map;for(const u of c.events){const h=u.projectName||u.projectSlug||"Global",i=r.get(h)||[];i.push(u),r.set(h,i)}return Array.from(r.entries()).map(([u,h])=>{var i;return n.jsxs("div",{className:"mb-1 last:mb-0",children:[n.jsxs("div",{className:"flex items-center gap-1.5 mb-0.5",children:[n.jsx("div",{className:"w-2 h-2 rounded-full shrink-0",style:{backgroundColor:(i=h[0])!=null&&i.projectSlug?e.get(h[0].projectSlug)||"#6B7280":"#64748B"}}),n.jsx("span",{className:"text-[9px] font-medium text-slate-300",children:u}),n.jsxs("span",{className:"text-[8px] text-slate-600",children:["(",h.length,")"]})]}),h.slice(0,2).map(b=>n.jsxs("div",{className:"flex items-center gap-1.5 ml-3.5",children:[n.jsx("div",{className:"w-1 h-1 rounded-full shrink-0",style:{backgroundColor:fe[b.type]}}),n.jsx("span",{className:"text-[8px] text-slate-500 truncate",children:b.label})]},b.id)),h.length>2&&n.jsxs("span",{className:"text-[7px] text-slate-600 ml-3.5",children:["+",h.length-2," more"]})]},u)})})():n.jsxs(n.Fragment,{children:[c.events.slice(0,4).map(r=>n.jsxs("div",{className:"flex items-center gap-1.5",children:[n.jsx("div",{className:"w-1.5 h-1.5 rounded-full shrink-0",style:{backgroundColor:fe[r.type]}}),n.jsx("span",{className:"text-[9px] text-slate-400 truncate",children:r.label})]},r.id)),c.events.length>4&&n.jsxs("span",{className:"text-[8px] text-slate-600",children:["+",c.events.length-4," more"]})]})})]})}),n.jsx("div",{className:"absolute bottom-2 right-3 text-[9px] text-slate-600 pointer-events-none",children:"Drag to rotate · Scroll to zoom"})]})}function ve(H,e={}){const p=e.fontSize??32,l=e.color??"#94a3b8",a=document.createElement("canvas");a.width=128,a.height=48;const o=a.getContext("2d");o.clearRect(0,0,a.width,a.height),o.font=`${p}px -apple-system, BlinkMacSystemFont, sans-serif`,o.textAlign="center",o.textBaseline="middle",o.fillStyle=l,o.fillText(H,a.width/2,a.height/2);const m=new Ge(a);m.minFilter=We;const c=new $e({map:m,transparent:!0});return new Ke(c)}export{rt as ActivityHeatmap3D};
